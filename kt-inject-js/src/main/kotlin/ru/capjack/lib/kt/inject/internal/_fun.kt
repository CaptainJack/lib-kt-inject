...
import ru.capjack.lib.kt.reflect.accessName
import ru.capjack.lib.kt.reflect.qualifiedNameRef
	...
	members.forEach { m ->
		...
		proxyClass.prototype[m.member.accessName] =
			if (a.isEmpty()) {
				...
				{ injector.create(t, a.map { it.invoke(injector, js("arguments").unsafeCast<Array<out Any>>()) }.toTypedArray()) }
			}
	}
